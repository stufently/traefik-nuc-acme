...
🚨 `csr-dump` обязан читать ИМЕННО ПЕРЕХВАЧЕННОЕ ТЕЛО запроса. Запрещено брать
CSR из любого файла, который написал наш же Traefik или наш же код, и запрещено
строить CSR заново в скрипте. Иначе критерий снова будет проверять наш
собственный артефакт, а не то, что реально ушло в CA, — то есть ровно ту дыру,
ради закрытия которой добавляется прокси. Если перехват почему-то невозможен —
остановись и доложи, подменять источник нельзя.
