...
- **AC-013** — проверки, закрывающие дыры перекрёстного обзора, зелёные
  (полнота сравнения пинов, сохранность существующего бандла при отказе,
  уборка TMPDIR на успехе и на отказе, совпадение пути монтирования из README с
  путём в пресете, объявленность entryPoint у challenge, границы аргументов в
  обвязке Go). Каждая проверка убивает свою порчу — доказано прогоном:
  `bash -c './scripts/nuc_preset_checks.sh'`

  Не закрыт сознательно один выживший из обзора: **M04, неатомарная запись** в
  файл назначения. Чтобы его поймать, нужен сбой ПОСРЕДИ финальной записи —
  инъекция `cat` через PATH, как в обзоре. Цена регрессии: при сбое записи
  портится рабочий бандл, чинится повторным запуском скрипта; сам скрипт пишет
  через staging и rename. Это оценённый пробел, а не долг — не тратить на него
  веху.
